...
# -*- coding: utf-8 -*-
# ---------------------------------------------------------------
# 캘리브레이션 사진 촬영 도구 (Ubuntu 호환)
# - 기능은 기존과 동일
# - Windows: CAP_DSHOW + waitKeyEx / Linux(Ubuntu): CAP_V4L2 + waitKey
# ---------------------------------------------------------------
...
_CAP_BACKEND = cv2.CAP_DSHOW if _IS_WINDOWS else cv2.CAP_V4L2

def ensure_dirs():
    # cam1, cam2가 따로 잡으면 각각 cam1, cam2로 들어가지는데,
    # 두 카메라에 체스보드가 동시에 잡히면 pairs로 들어가짐.
...
def main():
    ...
    # 카메라 열기 (기존 인덱스 유지: 1, 2)
    cam1 = cv2.VideoCapture(1, _CAP_BACKEND)
    cam2 = cv2.VideoCapture(2, _CAP_BACKEND)

    # 해상도/FPS 고정 (Ubuntu의 V4L2에서도 MJPG가 보통 안정적)
    W, H, FPS = 640, 480, 30
    ...
    # 장치 오픈 확인 (Ubuntu에선 반드시 먼저 체크)
    if not cam1.isOpened() or not cam2.isOpened():
        print("카메라를 열 수 없습니다. (인덱스/권한/연결 확인)")
        # 리눅스에서 권한 문제면: sudo usermod -aG video $USER && 재로그인
        return

    # (선택) 버퍼를 최소화해서 지연 줄이기 – 일부 드라이버만 지원하므로 예외 무시
    try: cam1.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    except Exception: pass
    try: cam2.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    except Exception: pass
    ...
    # 미리보기 창
    cv2.namedWindow('Camera 1 (Left) | Camera 2 (Right) - Focus this window for keys', cv2.WINDOW_NORMAL)
    ...
    while True:
        ...
        combined = np.hstack((disp2, disp2))  # 좌: cam1, 우: cam2  (원 코드 유지)
        ...
        # OS별 키 입력 처리 (Windows: waitKeyEx / Linux: waitKey)
        if _IS_WINDOWS and hasattr(cv2, "waitKeyEx"):
        ...
        if key == ord('1'):
            ...
            def quick_find(frame):
                ...
                found, _ = find_chessboard(gray, chessboard_size)
                return found
            ...
            cv2.putText(vis, status, (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (0,255,0) if (found1 and found2) else (0,0,255), 1, cv2.LINE_AA)
    ...
    try:
        save_q.join()  # 큐 비우기(원 코드 의도 범위 내에서 안전)
    except Exception:
        pass
