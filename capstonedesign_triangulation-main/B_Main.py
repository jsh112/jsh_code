...
# === MediaPipe 모듈 ===
from B_Mediapipe import PoseTracker, TouchCounter, draw_pose_points
...
def main():
    ...
    ap.add_argument("--port", default="COM10", help="시리얼 포트 (예: COM8)")
    ap.add_argument("--baud", type=int, default=115200)
    ap.add_argument("--center", action="store_true", help="시작 시 1회 센터 이동")
    ap.add_argument("--pitch", type=float, required=True, help="초기 pitch 값(도)")
    ap.add_argument("--yaw",   type=float, required=True, help="초기 yaw 값(도)")
    ap.add_argument("--laser_on",  action="store_true", help="시작 시 레이저 ON")
    ap.add_argument("--laser_off", action="store_true", help="시작 시 레이저 OFF")
    ap.add_argument("--no_auto_advance", action="store_true", help="자동 넘김 비활성화")
    ...
    print("[ΔAngles] (i -> i+1):  dyaw(deg), dpitch(deg), 3D_angle(deg)")
                ...
                for name, (x, y) in coords.items():
                    ...
                    if inside:
                        ...
                        if name in pose.success_parts:
                            ...
                            if triggered and not any(r[1] == tid for r in grip_records):
                                grip_records.append([name, tid, cx, cy])
                                filled_ids.add(tid)
                                with open("grip.txt", "a", encoding="utf-8") as f:
                                    f.write(f"[SUCCESS] Hold ID={tid}, Pos=({cx},{cy}), by {name}\n")
                                print(f"[SUCCESS] Hold ID={tid}, Pos=({cx},{cy}), by {name}")
                        ...
                        # 차폐 파트
                            ...
                            if not blocked_state.get(key, False):
                                with open("blocked.txt", "a", encoding="utf-8") as f:
                                    f.write(f"[BLOCKED] Hold ID={tid}, Pos=({cx},{cy}), blocked by {name}\n")
    ...
    print(f"[Info] 그립 CSV: {CSV_GRIPS_PATH} (총 {len(grip_records)}개)")
